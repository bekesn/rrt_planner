...
{
    // Limit v
    float vMax = vxLimit(vehicleParam);
    if(v_ > vMax) v_ = vMax;
    else if(v_ < 1) v_ = 1;
...
{
    return sqrt(vehicleParam->wheelBase * vehicleParam->maxLatAccel / delta_);
}
...
{
    return sqrt(vehicleParam->wheelBase * vehicleParam->maxLatAccel / (delta_ + timeStep * ddelta));
}
